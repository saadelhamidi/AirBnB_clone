...
```
validate.py index.html
```

Multiple files:

```
validate.py index.html header.html styles/common.css
```
...
def __print_stdout_now(msg):
    """Print message in STDOUT
    """
    sys.stdout.buffer.write(msg.encode('utf-8'))


def __print_stderr_now(msg):
    """Print message in STDERR
    """
    sys.stderr.buffer.write(msg.encode('utf-8'))


def __is_empty_now(file):
    if os.path.getsize(file) == 0:
        raise OSError("File '{}' is empty.".format(file))


def __validate_now(file_path, type):
    """
